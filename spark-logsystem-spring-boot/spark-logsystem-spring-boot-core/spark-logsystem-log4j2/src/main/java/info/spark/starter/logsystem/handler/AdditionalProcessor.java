...
import info.spark.starter.logsystem.enums.LogAppenderType;
import info.spark.starter.logsystem.listener.SparkLoggingListener;
...
public class AdditionalProcessor extends AbstractPropertiesProcessor {
     ...
     * @see SparkLoggingListener#initializeSystem
    ...
    private void setLogConfigFile(boolean isLocalLaunch) {
        ...
        JustOnceLogger.printOnce(AdditionalProcessor.class.getName(), "spark.logging.config=" + logConfig);
    }

    /**
     * 设置是否显示跳转信息 (spark.logging.enable-show-location), 用于本地开发快速跳转到日志输出位置
     *
     * @param isLocalLaunch is local launch
     * @since 1.0.0
     */
    ...
    private void setLogAppName() {
        ...
        // spark.logging.app-name
        this.setSystemProperty(StringUtils.isBlank(applicationName) ? "Please-Inherit-SparkStarter" : applicationName, Constants.APP_NAME);
