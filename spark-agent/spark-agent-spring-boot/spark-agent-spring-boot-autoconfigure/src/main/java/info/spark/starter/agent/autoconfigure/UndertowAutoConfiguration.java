...
/**
 * <p>Description: undertow 配置类, 主要配置 access.log </p>
 *
 * @author dong4j
 * @version 1.0.0
...
public class UndertowAutoConfiguration implements SparkAutoConfiguration, WebServerFactoryCustomizer<UndertowServletWebServerFactory> {
     ...
     */
    @Contract(pure = true)
    public UndertowAutoConfiguration(AgentProperties properties) {
    ...
    static class CustomRequestDumpingHandler implements HttpHandler {
         ...
         */
        @Contract(pure = true)
        CustomRequestDumpingHandler(HttpHandler next, AgentProperties properties) {
            this.next = next;
            this.properties = properties;
        }
