...
class HttpServerHandler extends SimpleChannelInboundHandler<FullHttpRequest> {
     ...
     * @param config             config
     * @param eventExecutorGroup event executor group
     * @param isCors             is cors
     * @since 2022.1.1
     */
    ...
    private void handleHttpRequest(ChannelHandlerContext ctx, FullHttpRequest req) {
        ...
        // Handshake
        WebSocketServerHandshakerFactory wsFactory = new WebSocketServerHandshakerFactory(getWebSocketLocation(req), subprotocols, true,
                                                                                          config.getmaxFramePayloadLength());
        ...
        if (handshaker == null) {
            ...
            if (config.getReaderIdleTimeSeconds() != 0 || config.getWriterIdleTimeSeconds() != 0 || config.getAllIdleTimeSeconds() != 0) {
                pipeline.addLast(new IdleStateHandler(config.getReaderIdleTimeSeconds(), config.getWriterIdleTimeSeconds(),
