...
public class JacksonValueDecoder extends SparkAbstractValueDecoder implements ValueDecoderRegister {
    /** INSTANCE */
    public static final JacksonValueDecoder INSTANCE = new JacksonValueDecoder();
    /** 反序列化时使用序列化的 mapper */
    private static final ObjectMapper MAPPER = JacksonValueEncoder.OBJECT_MAPPER;
    ...
    @SneakyThrows
    @Nullable
    private Object deserialize(byte[] source, boolean isWrapper) {
        ...
        if (isWrapper) {
            ...
            if (value == null) {
                ...
                return JsonUtils.parse(MAPPER, source, new TypeReference<CacheValueHolder<?>>() {});
            }
        } else {
            return JsonUtils.parse(MAPPER, source, new TypeReference<Object>() {});
        }
    }
