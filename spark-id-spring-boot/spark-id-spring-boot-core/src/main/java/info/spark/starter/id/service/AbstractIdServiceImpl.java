...
import info.spark.starter.id.converter.IdConverter;
import info.spark.starter.id.converter.IdConverterImpl;
import info.spark.starter.id.entity.Id;
import info.spark.starter.id.entity.IdMeta;
import info.spark.starter.id.entity.IdMetaFactory;
...
public abstract class AbstractIdServiceImpl implements IdService {
     ...
     */
    @Contract(pure = true)
    protected AbstractIdServiceImpl() {
     ...
     * @since 1.5.0
     */
    protected AbstractIdServiceImpl(String type) {
     ...
     */
    @Contract(pure = true)
    protected AbstractIdServiceImpl(IdType type) {
        this.idType = type;
    }
    ...
    public void init() {
        ...
        if (this.idMeta == null) {
            ...
            if (this.idMeta.getTimeBits() == IdType.MAX_PEAK.getSize()) {
                ...
                throw new ServiceInternalException("Init Error. The time bits in IdMeta should be set to 30 or 40!");
            }
        }
    ...
    public long makeId(long version,
        ...
        return new IdConverterImpl(IdType.parse(type)).convert(id);
    }
