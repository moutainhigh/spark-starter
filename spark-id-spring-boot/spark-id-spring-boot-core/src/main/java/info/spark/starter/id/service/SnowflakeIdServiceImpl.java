...
import info.spark.starter.id.exception.IdWorkerException;
...
public class SnowflakeIdServiceImpl implements IdService {
    ...
    public SnowflakeIdServiceImpl(Date epochDate, long machineId, long dataCenterId, boolean isUseSystemClock) {
        ...
        if (machineId > this.maxMachineId || machineId < 0) {
            throw new IllegalArgumentException(StrFormatter.format("worker Id can't be greater than {} or less than 0",
                                                                   this.maxMachineId));
        }
        if (dataCenterId > this.maxDataCenterId || dataCenterId < 0) {
            throw new IllegalArgumentException(StrFormatter.format("datacenter Id can't be greater than {} or less than 0",
    ...
    public synchronized long nextId() {
        ...
        if (timestamp < this.lastTimestamp) {
            // 如果服务器时间有问题(时钟后退) 报错.
            throw new IllegalStateException(StrFormatter.format("Clock moved backwards. Refusing to generate id for {}ms",
    ...
    public Id expId(long id) {
        return new Id(this.getMachineId(id),
                      this.getSequence(id),
                      this.getGenerateDateTime(id),
                      DeployType.EMBED.getType(),
                      IdType.MAX_PEAK.getVaule(),
                      0);
    }
